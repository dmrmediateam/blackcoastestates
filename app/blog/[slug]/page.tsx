...
import { generateArticleSchema, generateBreadcrumbSchema, generateFaqSchema, generatePersonSchema } from '@/lib/schema'
    ...
    return {
      ...
      openGraph: {
        ...
        authors: post.authorSlug
          ? [`https://blog.blackcoastestates.com/author/${post.authorSlug}`]
          : undefined,
        tags: post.tags,
      ...
      alternates: {
        canonical: `https://blog.blackcoastestates.com/blog/${post.slug}`,
      },
    }
  } catch {
    return {}
          ...
          <div className="max-w-4xl mx-auto px-6">
            ...
            {author ? (
              <div className="flex items-start gap-4">
                <a href={`/author/${author.slug}`} className="flex-shrink-0 mt-0.5">
                  <AuthorAvatar src={author.image} name={author.name} size={52} />
                </a>
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <a
                      href={`/author/${author.slug}`}
                      className="text-sm font-medium text-bce-cream hover:text-bce-gold transition-colors"
                    >
                      {author.name}
                    </a>
                    <span className="text-bce-border" aria-hidden="true">·</span>
                    <span className="text-[0.6rem] tracking-[0.15em] uppercase text-bce-muted font-sans">
                      {author.title}
                    </span>
                    <span className="text-bce-border" aria-hidden="true">·</span>
                    <span className="text-xs text-bce-muted font-sans">
                      Published{' '}
                    ...
                    {post.lastModified && post.lastModified !== post.date && (
                      <>
                        <span className="text-bce-border" aria-hidden="true">·</span>
                        <span className="text-[0.6rem] tracking-[0.1em] uppercase text-bce-muted font-sans">
                          Updated{' '}
                          <time dateTime={post.lastModified}>
                            {format(new Date(post.lastModified), 'MMM d, yyyy')}
                          </time>
                        </span>
                      </>
                    )}
                  </div>
                  <p className="text-xs text-bce-cream/50 leading-relaxed max-w-lg font-sans">
                    {author.bio.split('.')[0]}.
                  </p>
                </div>
              </div>
